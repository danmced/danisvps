/*
  Capture Letter View Utility
  Generates shareable letter card images for Instagram Stories and downloads
  Uses canvas to render letter data as a pixel art styled card
  Design: 8-bit retro gaming aesthetic with pink/cream Valentine's theme
*/

export interface Letter {
  id: string;
  recipient: string;
  sender: string;
  message: string;
  createdAt: Date;
}

// Color palette matching the pixel art theme
const COLORS = {
  background: "#FFF5E1", // Cream
  cardBg: "#FFFFFF", // White
  primary: "#E63946", // Red
  secondary: "#FFB3D9", // Soft pink
  accent: "#FF6B9D", // Hot pink
  dark: "#2B2D42", // Dark outline
  border: "#E0E0E0", // Light border
  text: "#2B2D42", // Dark text
  mutedText: "#666666", // Muted text
};

/**
 * Generate a letter card image from letter data
 * @param letter - The letter object to render
 * @returns Promise<Blob> - The generated image as a blob
 */
export const generateLetterCardImage = async (letter: Letter): Promise<Blob> => {
  return new Promise((resolve, reject) => {
    try {
      const canvas = document.createElement("canvas");
      canvas.width = 1080;
      canvas.height = 1920;
      const ctx = canvas.getContext("2d");

      if (!ctx) {
        reject(new Error("Could not get canvas context"));
        return;
      }

      // Fill background with cream
      ctx.fillStyle = COLORS.background;
      ctx.fillRect(0, 0, canvas.width, canvas.height);

      // Draw outer thick border (dark outline)
      ctx.strokeStyle = COLORS.dark;
      ctx.lineWidth = 12;
      ctx.strokeRect(30, 30, canvas.width - 60, canvas.height - 60);

      // Draw inner red border
      ctx.strokeStyle = COLORS.primary;
      ctx.lineWidth = 6;
      ctx.strokeRect(50, 50, canvas.width - 100, canvas.height - 100);

      // Draw card background
      ctx.fillStyle = COLORS.cardBg;
      ctx.fillRect(70, 70, canvas.width - 140, canvas.height - 140);

      // Draw decorative pixel hearts in corners
      drawPixelHeart(ctx, 110, 110, 32, COLORS.primary);
      drawPixelHeart(ctx, canvas.width - 110, 110, 32, COLORS.secondary);
      drawPixelHeart(ctx, 110, canvas.height - 110, 32, COLORS.secondary);
      drawPixelHeart(ctx, canvas.width - 110, canvas.height - 110, 32, COLORS.primary);

      // Title
      ctx.fillStyle = COLORS.dark;
      ctx.font = "bold 48px 'Arial', sans-serif";
      ctx.textAlign = "center";
      ctx.fillText("VALENTINE'S", canvas.width / 2, 180);
      ctx.fillText("LETTER", canvas.width / 2, 240);

      // Decorative line
      ctx.strokeStyle = COLORS.accent;
      ctx.lineWidth = 3;
      ctx.beginPath();
      ctx.moveTo(150, 280);
      ctx.lineTo(canvas.width - 150, 280);
      ctx.stroke();

      // Content area
      const contentTop = 330;
      const contentBottom = canvas.height - 280;
      const contentLeft = 120;
      const contentRight = canvas.width - 120;

      // To/From section
      ctx.fillStyle = COLORS.dark;
      ctx.font = "bold 28px 'Arial', sans-serif";
      ctx.textAlign = "left";
      ctx.fillText(`TO: ${letter.recipient}`, contentLeft, contentTop + 50);
      ctx.fillText(`FROM: ${letter.sender}`, contentLeft, contentTop + 100);

      // Decorative separator
      ctx.strokeStyle = COLORS.secondary;
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(contentLeft, contentTop + 130);
      ctx.lineTo(contentRight, contentTop + 130);
      ctx.stroke();

      // Message text with word wrapping
      ctx.fillStyle = COLORS.text;
      ctx.font = "22px 'Arial', sans-serif";
      ctx.textAlign = "left";

      const lineHeight = 36;
      const maxWidth = contentRight - contentLeft - 40;
      const words = letter.message.split(" ");
      let line = "";
      let y = contentTop + 200;

      for (let i = 0; i < words.length; i++) {
        const testLine = line + words[i] + " ";
        const metrics = ctx.measureText(testLine);

        if (metrics.width > maxWidth && line) {
          ctx.fillText(line.trim(), contentLeft + 20, y);
          line = words[i] + " ";
          y += lineHeight;

          // Stop if we're running out of space
          if (y > contentBottom - 120) {
            ctx.fillText(line.trim() + "...", contentLeft + 20, y);
            break;
          }
        } else {
          line = testLine;
        }
      }

      // If we haven't exceeded space, draw the last line
      if (y <= contentBottom - 120 && line) {
        ctx.fillText(line.trim(), contentLeft + 20, y);
      }

      // Footer with branding - @danmced
      ctx.fillStyle = COLORS.accent;
      ctx.font = "20px 'Arial', sans-serif";
      ctx.textAlign = "center";
      ctx.fillText("@danmced", canvas.width / 2, canvas.height - 80);

      // Date at bottom
      const dateStr = new Date(letter.createdAt).toLocaleDateString("en-US", {
        month: "short",
        day: "numeric",
        year: "numeric",
      });
      ctx.fillStyle = COLORS.mutedText;
      ctx.font = "16px 'Arial', sans-serif";
      ctx.fillText(dateStr, canvas.width / 2, canvas.height - 40);

      // Convert canvas to blob
      canvas.toBlob((blob) => {
        if (blob) {
          resolve(blob);
        } else {
          reject(new Error("Failed to generate canvas blob"));
        }
      }, "image/png");
    } catch (error) {
      reject(error);
    }
  });
};

// Draw a simple pixel heart shape
const drawPixelHeart = (
  ctx: CanvasRenderingContext2D,
  x: number,
  y: number,
  size: number,
  color: string
) => {
  ctx.fillStyle = color;
  const s = size / 8; // Scale unit

  // Heart shape using pixels
  ctx.fillRect(x - 2 * s, y - s, s, s); // Top left bump
  ctx.fillRect(x + s, y - s, s, s); // Top right bump
  ctx.fillRect(x - 3 * s, y, 7 * s, s); // Middle row
  ctx.fillRect(x - 3 * s, y + s, 6 * s, s); // Lower middle
  ctx.fillRect(x - 2 * s, y + 2 * s, 4 * s, s); // Lower
  ctx.fillRect(x - s, y + 3 * s, 2 * s, s); // Bottom point
};

/**
 * Download the letter card as an image
 * @param letter - The letter object to download
 */
export const downloadLetterView = async (letter: Letter) => {
  try {
    const blob = await generateLetterCardImage(letter);
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `valentine-letter-${letter.id}.png`;
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
    URL.revokeObjectURL(url);
  } catch (error) {
    console.error("Error downloading letter view:", error);
    throw error;
  }
};

/**
 * Share the letter card to Instagram Stories
 * @param letter - The letter object to share
 */
export const shareLetterViewToInstagramStories = async (letter: Letter) => {
  try {
    const blob = await generateLetterCardImage(letter);
    const file = new File([blob], `valentine-letter-${letter.id}.png`, {
      type: "image/png",
    });

    // Check if we're on mobile
    const isMobile = /iPhone|iPad|iPod|Android/i.test(navigator.userAgent);

    if (isMobile && navigator.share) {
      // On mobile with Web Share API support
      try {
        await navigator.share({
          files: [file],
          title: "Valentine's Letter",
          text: `Check out this Valentine's letter from ${letter.sender} to ${letter.recipient}! 💕 @danmced`,
        });
      } catch (shareError: unknown) {
        // User cancelled share or error occurred
        if (shareError instanceof Error && shareError.name !== "AbortError") {
          console.error("Share API error:", shareError);
          // Fallback to download
          await downloadLetterView(letter);
        }
      }
    } else if (isMobile) {
      // Mobile without Web Share API - create Instagram Stories deep link
      const url = URL.createObjectURL(blob);
      const instagramStoriesUrl = `instagram://stories/share?media_type=image&source_url=${encodeURIComponent(url)}`;

      // Try to open Instagram Stories
      const opened = window.open(instagramStoriesUrl, "_blank");

      // If Instagram app didn't open, fall back to download
      if (!opened || opened.closed) {
        URL.revokeObjectURL(url);
        await downloadLetterView(letter);
      }
    } else {
      // Desktop - download the image
      await downloadLetterView(letter);
    }
  } catch (error) {
    console.error("Error sharing letter view to Instagram Stories:", error);
    throw error;
  }
};
